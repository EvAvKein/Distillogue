...
const UserPatchRequest = Joi.object<Class.UserPatchRequest<editableUserData>>({
	dataName: Joi.valid(...arrOfEditableUserData).required(),
	newValue: Joi.required()
		...
		.when("dataName", {is: "presets", then: SharedSchema.User.UserData.extract("presets")})
...
const NodeCreationRequest = Joi.object<Class.NodeCreationRequest>({
	invitedOwnerIds: Joi.array().items(Joi.string()),
	title: SharedSchema.Post.Node.title,
	body: SharedSchema.Post.Node.body,
	deletedDraftIndex: Joi.number().integer().greater(-1).less(3),
	config: SharedSchema.Post.PostConfig.optional(),
	nodePath: Joi.array().min(1).items(Joi.string()),
}).required();

const NodeInteractionRequest = Joi.object<Class.NodeInteractionRequest>({
	nodePath: Joi.array().required().min(1).items(Joi.string()),
	interactionType: Joi.valid(...arrOfInteractionTypes).required(),
	interactionData: Joi.required()
		.when("interactionType", {is: "reply", then: NodeCreationRequest.required()})
		.when("interactionType", {
			is: "vote",
			then: Joi.object({
				voteDirection: Joi.valid("up", "down").required(),
				newVoteStatus: Joi.boolean().required(),
			}).required(),
		}),
}).required();
