<template>
	<section>
		<div v-if="title || content.editable" class="titleBox">
			<component v-if="title" :is="'h' + (title?.headingRank || 4)" class="title">
				{{ props.title?.text }}
			</component>
			<div v-if="content.editable" class="buttonBox">
				<button v-show="!editMode" @click="enterEditMode" class="core_backgroundButton">✎</button>
				<button v-show="editMode" @click="submitEdit" class="core_backgroundButton">✔</button>
				<button v-show="editMode" @click="exitEditMode" class="core_backgroundButton">✘</button>
			</div>
		</div>
...
<script setup lang="ts">
	...
	async function submitEdit() {
		...
		notification.value.desirabilityStyle = undefined;
		const editResponse = await jsonFetch(
			"PATCH",
			props.content.editable!.editAuthAddress,
			[
				{
...
<style scoped>
	...
	p {
		...
		width: calc(
			100% - 1.1em
		); /* 1em should be the correct value considering the margins... but evidently, for whatever reason, it isnt */
		margin: 0.5em;
		border: 0;
		outline: none;
