...
<script setup lang="ts">
	...
	function doubleDigiter(number: string | number) {
		return number < 10 ? "0" + number : number;
	}
	const absoluteTimestamp = `${doubleDigiter(dateObject.getHours())}:${doubleDigiter(
		dateObject.getMinutes()
	)}, ${doubleDigiter(dateObject.getDate())}.${doubleDigiter(dateObject.getMonth() + 1)}.${dateObject
		.getFullYear()
		.toString()
		.slice(-2)}`;
	...
	function timestamp(pastUnix: number) {
		...
		if (unixDiff > day) {
			return Math.floor(unixDiff / day) + "d ago";
		}
		if (unixDiff > hour) {
			return Math.floor(unixDiff / hour) + "h ago";
		}
		if (unixDiff > minute) {
			return Math.floor(unixDiff / minute) + "m ago";
		}
		if (unixDiff > 15) {
			return unixDiff + "s ago";
		}
		return "Now";
	}
