...
describe("Create access & interactions test posts", () => {
	...
	it("Create interactions-testings & access-testing posts", () => {
		cy.submitPost(postTitle, postBody, () => {
		...
		cy.submitPost("[NON-PUBLIC] " + postTitle, "[NON-PUBLIC] " + postBody, () => {
			cy.get("button").contains("Everything").click();
	...
	it("Enter access-testing post & copy URL", () => {
		...
		cy.get("ol")
			.contains("li", "[NON-PUBLIC] " + postTitle)
			.click();
...
describe("Replies & deep interaction", () => {
	...
	function replyAndValidateNode(repliedNodeByTitlesPath: string[], replyTitle: string, replyBody: string) {
		...
		cy.get("@node").parent().find(".replies").contains("h3", replyTitle).parent().parent().contains("p", replyBody);
	...
	it("Reply to central node", () => {
		replyAndValidateNode([postTitle], "1st Reply to Central", randomNodeBody());
	});
	it("Another reply to central node", () => {
		replyAndValidateNode([postTitle], "2nd Reply to Central", randomNodeBody());
	});
	it("Reply to reply", () => {
		replyAndValidateNode([postTitle, "1st Reply to Central"], "1st Reply to 1st Reply to Central", randomNodeBody());
	});
	...
	it("Another reply to reply", () => {
		replyAndValidateNode([postTitle, "2nd Reply to Central"], "1st Reply to 2nd Reply to Central", randomNodeBody());
	});
...
describe("Voting", () => {
	...
	function validateVoteInterface(userVote: "up" | "down" | "none", voteCount: number) {
		// could've removed the voteCount parameter and instead let it be based on the userVote, but this separation supports testing with multiple voters (which doesnt currently seem necessary but i might in future switch to)
		cy.get(upvoteSelector)
			.first()
			.should(userVote === "up" ? "have.class" : "not.have.class", "voted");
		cy.get(voteNumbSelector).first().should("have.text", voteCount);
		cy.get(downvoteSelector)
			.first()
			.should(userVote === "down" ? "have.class" : "not.have.class", "voted");
	}
